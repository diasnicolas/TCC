...
\item [Capítulo 4] este capítulo descreve os principais conceitos de meta programação, reflexão e metadados. Aborda de forma prática o uso de anotações (metadados) na linguagem Java.
...
A figura \ref{fig:fluxogramaAgent} mostra todos os processos realizados pela JVM quando é chamado um \textit{Agent} junto à chamada de um sistema. O fluxograma foi desenvolvido utilizando ferramentas disponibilizadas em \textit{http://cacoo.com}.
...
Após a cópia dos campos para a classe alvo, é necessário realizar a integração entre o \textit{advice} e o método interceptado. è dada a partida para essa integração através da chamada ao método \texttt{addAdviceMethod}, cuja responsabilidade é fazer uma cópia do \textit{advice} e repassar esta cópia para o método correspondente ao tipo de \textit{advice}. Os métodos que combinam cada tipo de \textit{advice} são: \texttt{addBefore}, \texttt{addAfter}, \texttt{AddAround}. Na figura \ref{fig:addadvicemethod} é mostrado a implementação do método \texttt{addAdvicemethod}.
...
Para realizar a combinação do método com um \textit{advice} do tipo \textit{before} é utilizado o método \texttt{addBefore}. Primeiramente deve-se adicionar a cópia do \texttt{advice} que veio por parâmetro para a classe alvo, visando realizar chamadas diretas a esta cópia. O nome do método copiado é formado da seguinte maneira: \texttt{aspecto\$advice\$método}. Para adicionar uma chamada à cópia do \textit{advice} antes da execução do método interceptado é necessário utilizar o método \texttt{insertBefore} presente na classe \texttt{CtMethod} do Javassist. A chamada para o método \texttt{insertBefore} deve ser realizada pela instância do método interceptado. Na figura \ref{fig:addbefore} está a implementação do método \texttt{addBefore} da classe \texttt{Weaver}.
...
Pode-se notar que para instanciar a classe \texttt{Params} na \texttt{linha 150} é passado dois parâmetros, o primeiro (\textit{\$args}) representa um objeto do tipo \texttt{Objeto[]} que possui os valores dos parâmetros e o segundo (\$sig) é do tipo \texttt{Class[]} que possui os tipos de cada parâmetro. Esta sintaxe utilizada em forma de \texttt{String} é traduzida para \textit{bytecode} pelo Javassist.
 
 A implementação do método \texttt{addAfter} é praticamente a mesma do método \texttt{addBefore}, sendo que a única diferença é que \texttt{addAfter} utiliza o método \texttt{insertAfter} para adicionar código depois, e o \texttt{addBefore} utiliza o método \texttt{insertBefore} para adicionar depois. A implementação de \texttt{addAfter} pode ser vista na figura \ref{fig:addafter}.
 
 
\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.5]{figuras/implementacao/addafter.png}
