...
\title{AopJ: Framework AOP utilizando técnicas de Bytecode Engineering}

\author{Bento}{Nicolas Dias}

\advisor[Prof. Esp.]{Torres}{Márcio}
...
\keyword{AOP, Bytecode Engineering, Meta Programming, Framework, SoC}
...
Monografia sob o título \textit{"AopJ: Framework AOP utilizando técnicas de Bytecode Engineering"}, defendida por Nicolas Dias Bento e aprovada em \_\_\_ de \_\_\_\_\_\_\_\_\_\_\_ de \_\_\_\_\_, em Rio Grande, estado do Rio Grande do Sul, pela banca examinadora constituída pelos professores:
...
\begin{abstract}
The software development industry is one of the sectors that has grown over the recent years. Because of this high demand, it is necessary to use techniques and methodologies that support the maintenance, modularization and the greater reuse of code possible. The Aspect Oriented Programming (AOP) paradigm is used in conjunction with the Object Oriented Programming (OOP) paradigm, in order to obtain a 100\% modularized system, separating crosscutting concerns (logging, exception handling, validation, etc.) of core interests (business logic). Thus the cross concentrated interests capitals in "aspects" using AOP, and the core interests in "classes" using OOP. This work aims to develop a framework, able to implement by techniques of Bytecode Engineering the main concepts of AOP, order use it with OOP to solve the problems found modularization currently targeting the encapsulation of crosscutting concerns. It is also developing a case study of instrumentation, in order to demonstrate the efficiency and ease of use of the framework for the development and integration of aspects and classes. On completion of this work is presented an open source framework that will serve as a basis for other studies focused on AOP and Bytecode Engineering.
\end{abstract}
}
...
\item [Capítulo 7 AOPJ: O FRAMEWORK:] este capítulo descreve todas as características do projeto, desde a parte de análise até a implementação.
...
\begin{description}
\item [ClassFileTransformer (java.lang.instrument)] Esta é uma \texttt{interface} que deve ser implementada por classes que desejam ser responsáveis pela modificação de outras classes antes do carregamento para a máquina virtual.
...
O diagrama de sequência foi utilizado para demonstrar a forma como se dá a chamada dos métodos durante o processo inicialização do \textit{Agent} até a parte de modificação (combinação de classes e aspectos) do \textit{bytecode} das classes interceptadas. Este diagrama pode ser visto na figura \ref{fig:sequenciaCombinador}.
...
\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.6]{figuras/implementacao/proceed.png}
...
Para demonstrar os resultados obtidos após a combinação entre um aspecto e uma classe, será utilizado uma simulação de como estaria o código fonte da classe alvo após a combinação.

\subsubsection{Antes da combinação}

A figura \ref{fig:classaspecto} mostra o aspecto \texttt{TratamentoErro} e a classe \texttt{Cliente} antes da combinação, de forma que a regra de combinação foi a chamada ao método \texttt{salvar}.

\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.4]{figuras/implementacao/utilizandoaspecto.png}
	\caption{Comunicação entre classe e aspecto.}
	\label{fig:classaspecto}
\end{figure}

\subsubsection{Após a combinação}

O código fonte que equivale ao \textit{bytecode} da classe \texttt{Cliente} após o processo de combinação entre classe e aspecto utilizando o tipo de \textit{advice around} pode ser vista na figura \ref{fig:combinacaoAroundfonte}.

\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.4]{figuras/implementacao/combinacaoAround.png}
	\caption{Código fonte equivalente ao \textit{bytecode} da classe após combinação.}
	\label{fig:combinacaoAroundfonte}
\end{figure}

Pode-se perceber claramente na figura \ref{fig:combinacaoAroundfonte} que o método \texttt{around} presente no aspecto é copiado para a classe alvo, que neste caso é a classe \texttt{Cliente}. Também é realizado uma cópia do método alvo, que neste caso é o método \texttt{salvar}. Nota-se que a implementação do método \texttt{salvar} é alterada, visando a preparação dos parâmetros necessários para realizar a chamada ao método \texttt{tratamentoerro\_around}.
...
\item [Build] é o processo de compilação de um \textit{software}.
\end{description}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=455px,height=425px,angle=90]{diagramas/Framework_AOP-completo.png}
	\caption{Diagrama de classes completo do framework.}
\end{figure}


\chapter{Processo de build: Ant}

Ant é uma das ferramentas de \textit{build} mais famosas utilizadas em IDE's para Java. Através do Ant é possível invocar tarefas utilizando \textit{tags} XML (\textit{eXtensible Markup Language}). Estas \textit{tags} são descritas em um arquivo cujo nome é \textit{build.xml}, que deve estar localizado dentro do projeto \cite{ant} .

O arquivo \textit{build.xml}, cujo conteúdo está presente na figura \ref{fig:buildframework}, é responsável pela compilação e geração do arquivo \textit{.jar} do \textit{framework}.

\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.8]{figuras/implementacao/build-framework.png}
	\caption{Arquivo de build do \textit{framework}.}
	\label{fig:buildframework}
\end{figure}


O arquivo \textit{build.xml}, cujo conteúdo está presente na figura \ref{fig:buildestudocaso}, é responsável pela compilação e execução do estudo de caso utilizado o \textit{framework}, para combinação entre aspectos e classes.

\begin{figure}[t]
	\centering
	\includegraphics[scale=0.7]{figuras/implementacao/build-estudocaso.png}
	\caption{Arquivo de build do \textit{estudo de caso}.}
	\label{fig:buildestudocaso}
\end{figure}
